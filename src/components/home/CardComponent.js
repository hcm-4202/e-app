...
function CardComponent({products}){
    ...
    return <Box sx={{display:'flex',flexWrap:'wrap'}}>
         {products.map((product)=>{
            let count =0
           return  <Card sx={{ width:'350px', margin:'20px', height:'400px', textAlign:'center' }}>
        ...
        <Typography sx={{fontWeight:'bold'}}>{`Price :${product.price}`}</Typography>
           ...
           {count ==0 ? <Button sx={{border: '1px solid gray'}} onClick={()=>dispatch(add(product))} >Add to cart <Typography sx={{marginLeft:'5px', marginTop:'5px'}}><ShoppingCartOutlinedIcon/></Typography> </Button>
            : <Box sx={{display:'flex' ,border:'1px solid gray', width:'150px', marginLeft:'90px'}}>
